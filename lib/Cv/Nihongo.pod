...
C<Createなんとか()> で作成できます。

 my $img = Cv->CreateImage([ 320, 240 ], IPL_DEPTH_8U, 3);
 my $mat = Cv->CreateMat(240, 320, CV_8UC4);

オブジェクトを作るには C<new> も使えます。CreateImage() と CreateMat()
は、上の例のとおり呼び出し形式が違いますが。C<new> は同じです。
...
C<CvMatND>, C<CvSparseMat> など、いろいろな型があります。Cv では、この
データ型をクラス名に対応させています。対応は、名前から推測できるもので、
たとえば、C<CvImage> は C<Cv::Image> に、C<CvMat> は C<Cv::Mat> に対応
しています。C<Cv> のオブジェクトは、これらのデータのポインタをクラス名
で bless しただけのものです。

OpenCV の画像やマトリクス C<CvMat>、C<CvImage> は、C<CvArr> から導出さ
れた形がとられており、いろいろな画像やマトリクスは C<CvArr> を介して関
数に渡されます。従って、Cv にもスーパクラス C<Cv::Arr> を作り、メソッド
はここに置きました。もちろん例外もありますが。
