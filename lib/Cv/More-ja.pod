...
=item ContourArea()
 ...
 my $s = Cv->cvContourArea($points)
 my $s = Cv->cvContourArea($points, $slice)
...
この場合の面積 $s は、縦 x 横 (100x100) になります。
...
Affine() は、簡単に変換行列を作る方法がなかったので、その対処として作っ
たものでしたが、 new() を拡張したので、必要なら次の書き方でも同じことが
できるようになったと思います。
...
張したものです。$slice は、cvSlice() で作るか、単にはじめ $start と終り
$end を対にした配列のリファレンス [$start, $end] です。省略したときは
$arr のすべての要素を変換します。
