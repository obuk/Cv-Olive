...
確かに正しいのですが、動いていた MinAreaRect() が動かなくなります。
  ...
  Cv->boxPoints([Cv->minAreaRect(\@points)]);  # なんで書き直しするの?

簡単にしよう。良くしよう。そう思ったのに、面倒くさいことになってしまい
ました。やはりインターフェースの変更は厄介ですね。というわけで、コンテ
クトに合わせた戻値を使うときには、次のとおり明示することにします。

  use Cv::More qw(cs);                 # コンテクトに合った戻値を使う
  use Cv::More qw(cs-warn);            # そのときに、少しお節介する

2行目のお節介は、コンテクストに合わせた戻値を使うと動かなくなる可能性の
あるところ、つまりリストコンテクストでスカラを返すような場面を実行時に
みつけたときにメッセージを出力するものです。はじめに、2行目の
cs-verbose で確認し、その後、1行目の cs を指定するといいでしょう。

もし全体でこのような変更を加える場合はいいですが、そうでないときには、
import() と unimport() を使ってください。
