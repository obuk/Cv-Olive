...
です。そう考えてあたりを見渡すと、Perl は warnings を使ってこのような問
題に対処しているように見えます。

たとえば、MiAreaRect() は、呼び出されたコンテクストに関係なく、いつでも
スカラを返していました。サボっていました。では、呼ばれたコンテクストに
合わせた戻り値を返すように、正しいと思う変更を加えましょう。そうすると、
確かに正しいのですが、動いていた MinAreaRect() が動かなくなります。
...
ました。やはりインターフェースの変更は厄介ですね。

だからと行って warnings を使って問題があるところについて知らせるだけで
なく、いままでどおり、どのようなコンテクストでもスカラを返し、できるだ
け同じ動作を保つことにしましょう。XXXXX

  my @box = Cv->boxPoints(Cv->minAreaRect(\@points)); # 警告はあるが使える

  no warnings 'Cv::oldfashion';                       # 警告は止まる
  my @box = Cv->boxPoints(Cv->minAreaRect(\@points)); # 動かない
  my @box = Cv->boxPoints([Cv->minAreaRect(\@points)]); # 動く
...
が確認していないので、渡さないでください。

 CvBox2D cvMinAreaRect2(const CvArr* points, CvMemStorage* storage=NULL)

Cv-0.15 までは、Perl の点のリストを Cv::Seq::Point にして使っていました。
そのときはメモリストレージを使うこともあったと思いますが、本当に使われ
ていたのか確認していません。Cv-0.16 から Perl の点のリストを Cv::Mat に
直したので、おそらく渡しても使われないだろうと思います。
