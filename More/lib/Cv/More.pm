...
=item Cv::Mat->new(), Cv::MatND->new()

Cv::More sets the initial values to the object created in
Cv::Mat-E<gt>new and Cv::MatND-E<gt>new as follows.

  my $mat = Cv::Mat->new([], CV_32SC2,
    [$x1, $y1], [$x2, $y2], [$x3, $y3],
    );

The functions usually needs a parameter for the object size.  But when
the initial values are used, the size parameter accepts empty arrayref
[].
...
{ no warnings 'redefine'; *new = \&m_new }
...
{ no warnings 'redefine'; *new = \&s_new }
...
{ no warnings 'redefine'; *Set = *set = \&m_set }
...
# { no warnings 'redefine'; *Get = *get = \&m_get }
...
{
	*Cv::Seq::Point::ToArray = 
		*Cv::Seq::Point::CvtSeqToArray = 
		*CvtMatToArray =
		\&ToArray
}
...
=item PerspectiveTransform()
 ...
 my @src = ($pt11, $pt12, $pt13, ...);
 my $dst = Cv->PerspectiveTransform(\@src, $mat);                       # (1)
 my @dst = Cv->PerspectiveTransform(\@src, $mat);                       # (1')
 Cv->PerspectiveTransform(\@src, my $dst, $transmat);                   # (2)
...
Cv->FitEllipse2() returns CvBox2D same as cvFitEllipse2() defined in
the OpenCV reference.  If you use Cv::More with C<cs>, and calling
FitEllipse() in the list context, the return value of it is expanded
as follows:

 use Cv::More qw(cs);
 my ($center, $size, $angle) = Cv->FitEllipse2($points);

The following example shows how to draw a list of points using
FitEllipse2().
...
*Cv::FitEllipse2 = \&FitEllipse2;
*FitEllipse = \&FitEllipse2;
*Cv::FitEllipse = \&FitEllipse;
...
=item FindHomography()
 ...
 my @pts1 = ($pt11, $pt12, $pt13, ...);
 my @pts2 = ($pt21, $pt22, $pt23, ...);
 my $H = Cv->FindHomography(\@pts1, \@pts2, $method, $ransac, $status); # (1)
 Cv->FindHomography(\@pts1, \@pts2, my $H, $method, $ransac, $status);  # (2)
...
sub FindHomography {
	...
	unless (ref $_[0] && $_[0]->isa('Cv::Mat')) {
		if (defined $_[0]) {
			unshift(@_, Cv::Mat->new([3, 3], &Cv::CV_64F));
		} else {
			$_[0] = Cv::Mat->new([3, 3], &Cv::CV_64F);
		}
	}
	unshift(@_, $srcPoints, $dstPoints);
